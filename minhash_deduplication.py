...
class DuplicationIndex:
    def __init__(
        ...
        *,
        ...
        index_filepath: str = "/mnt/shared-storage/users/xzluo/dedup/minhash_lsh.pkl",
        clusters_filepath: str = "/mnt/shared-storage/users/xzluo/dedup/duplicate_clusters.pkl"
    ):
        ...
        self.clusters_filepath = clusters_filepath
        self._load_index()
        self._load_clusters()
    ...
    def _load_index(self):
        if os.path.exists(self.index_filepath):
            ...
            print(f"Loaded MinHashLSH index from {self.index_filepath}")
        else:
            print("Initialized new MinHashLSH index.")

    def _save_index(self):
        with open(self.index_filepath, "wb") as f:
            pickle.dump(self._index, f)
        print(f"Saved MinHashLSH index to {self.index_filepath}")

    def _load_clusters(self):
        if os.path.exists(self.clusters_filepath):
            with open(self.clusters_filepath, "r") as f:
                self._duplicate_clusters = defaultdict(set, pickle.load(f))
            print(f"Loaded duplicate clusters from {self.clusters_filepath}")
        else:
            print("Initialized new duplicate clusters.")

    def _save_clusters(self):
        with open(self.clusters_filepath, "wb") as f:
            pickle.dump(self._duplicate_clusters, f)
        print(f"Saved MinHashLSH index to {self.index_filepath}")
    ...
    def add(self, code_key: Tuple, min_hash: MinHash) -> None:
        ...
        The new key is either added to an existing cluster of one close match,
        ...
        Args:
            code_key (Tuple of (index, repo_name, path)):
                Theoretically any hashable key. Here we use a tuple to retrieve the information later.
    ...
    def save_all(self):
        """Save both the MinHashLSH index and duplicate clusters."""
        self._save_index()
        self._save_clusters()

    def save_duplicate_clusters(self, filepath) -> None:
        """Save duplicate clusters to a specified filepath."""
...
def make_duplicate_clusters(dataset_iterator: Type[Dataset], jaccard_threshold: float, duplication_index: DuplicationIndex):
    ...
    for filename, min_hash in tqdm(ThreadedIterator(minhash_iter(enumerate(dataset_iterator)), max_queue_size=100)):
        duplication_index.add(filename, min_hash)
...
def find_extremes(cluster_list, dataset, jaccard_threshold):
    ...
    Args:
        ...
        dataset (Type[Dataset]):
            ...
            dataset is shared between all the processes using a global variable (any other way to share the dataset?),
            otherwise the multi processing is not sped up.
        jaccard_threshold (float):
...
def deduplicate_dataset(
    dataset: Type[Dataset], duplication_index: DuplicationIndex, jaccard_threshold: float = 0.85
    ...
    Args:
        ...
        duplication_index (DuplicationIndex):
            The duplication index to maintain state across chunks.
    ...
    """
    make_duplicate_clusters(dataset, jaccard_threshold, duplication_index)
    # After processing the entire dataset chunk, save the index and clusters once
    duplication_index.save_all()
    
    duplicate_clusters = duplication_index.get_duplicate_clusters()
